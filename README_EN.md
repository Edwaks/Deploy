...
**Deploy Aisaray** is the release, deployment, and rollback environment for AiSaray products. It is designed for product owners, release engineers, and quality reviewers and makes a version’s path from release decision to verified outcome transparent.

- Public URL: https://aisaray.ru/
- Core entities: products, versions, releases, stages, checks, decisions, and rollbacks.
- Key capabilities: release registry, promotion stages, quality gates, release decision, deployment status, controlled rollback, outcome log.

## Why the Product Is Needed

A real workflow rarely consists of a single action. Users must understand the initial situation, gather information, choose the next step, coordinate a decision, and return to the result later. Deploy Aisaray provides one domain-specific environment for this work. It preserves context between stages and shows not only an individual item, but also its place in the overall process.

The product’s main value is not the number of screens, but workflow continuity. Products, versions, releases, stages, checks, decisions, and rollbacks are treated as parts of one user journey. New participants understand the current state faster, experienced specialists spend less time matching information manually, and managers gain a clear overview without requiring every step to be retold.
...
The roles Product owner, Release engineer, and Quality reviewer work with the same domain model from different perspectives. Consistent names for objects and states reduce ambiguity during handoffs. A responsible participant can reference a specific object and expected result instead of repeating the entire context.

## Main Sections

The public description covers these user capabilities:

- **Release registry** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Promotion stages** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Quality gates** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Release decision** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Deployment status** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Controlled rollback** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Outcome log** — a distinct part of the workflow, connected to the product model and a clear user outcome.
